...
static inline void rtl_sext(rtlreg_t* dest, const rtlreg_t* src1, int width) {
   *dest =c_sar(c_shl(*src1,32-width*8),32-width*8); // dest <- signext(src1[(width * 8 - 1) .. 0])
}

static inline void rtl_push(const rtlreg_t* src1) {
  reg_l(R_ESP)-=4; // esp <- esp - 4
  rtl_sm (&reg_l(R_ESP),4,src1);// M[esp] <- src1
}

static inline void rtl_pop(rtlreg_t* dest) {
  
   rtl_lm(dest,&reg_l(R_ESP),4);// dest <- M[esp]
   reg_l(R_ESP)+=4;// esp <- esp + 4
  
}

static inline void rtl_eq0(rtlreg_t* dest, const rtlreg_t* src1) {
  *dest = *src1 == 0 ? 1 : 0;// dest <- (src1 == 0 ? 1 : 0)
}

static inline void rtl_eqi(rtlreg_t* dest, const rtlreg_t* src1, int imm) {
  *dest = *src1 == imm ? 1 : 0;// dest <- (src1 == imm ? 1 : 0)
...
static inline void rtl_msb(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  *dest =c_shr(c_shl(*src1 ,32-width*8),31) ;// dest <- src1[width * 8 - 1]
  
}

static inline void rtl_update_ZF(const rtlreg_t* result, int width) {
  cpu.eflags.ZF =c_shr(c_shl(*result , 32 - width*8) ,32 -width*8) == 0 ? 1 : 0 ; // eflags.ZF <- is_zero(result[width * 8 - 1 .. 0])
}

static inline void rtl_update_SF(const rtlreg_t* result, int width) {
  cpu.eflags.SF = c_shr(c_shl(*result ,32 -width*8),31) == 1 ? 1 : 0;// eflags.SF <- is_sign(result[width * 8 - 1 .. 0])
}
